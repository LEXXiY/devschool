...
<form id="form1" name="form1" method="post" style="margin-left: 30%; margin-top: 1%;"> <font size="5">
  <p>
    ...
    <input name="server_name" type="text" id="server_name" value="" placeholder="localhost">
  </p>
  <p>
    <label for="textfield2">User name:<br>
    </label>
    <input name="user_name" type="text" id="user_name" value="" placeholder="test">
  </p>
  <p>
    <label for="textfield3">Password:<br>
    </label>
    <input name="password" type="text" id="password" value="" placeholder="passhere">
  </p>
  <p>
    <label for="textfield4">Database:<br>
    </label>
    <input name="database" type="text" id="database" value="" placeholder="test">
  </p>
  <p>
...
(11,	'Черепаново');";
        ...
        if ($mysqli->select_db($_POST['database'])){
        ...
        if ($mysqli->query("CREATE DATABASE `{$_POST['database']}`")) {
            echo "База данных {$_POST['database']} успешно создана<br>";
            
            // Добавляем параметры подключения в файл конфигурации
            $settings = array($_POST['server_name'], $_POST['user_name'], $_POST['password'], $_POST['database']);
        ...
        if($mysqli->query("USE {$_POST['database']}")){
          
            $mysqli->multi_query($dump);
            
            echo 'Данные успешно добавлены в базу '.$_POST['database'].' <br> <a href="index.php">Перейти на сайт</a><br>';
        }
        else{
            echo 'Ошибка при добавлении данных в базу '.$_POST['database'];
        }
